// Generated by CoffeeScript 1.7.1
var draw_curved_rect, draw_keys;

draw_keys = function() {
  
  var canvas, ctx, i, key, key_positions, key_sizes, size_x, size_y, _i, _len, _ref, _results;
  canvas = document.getElementById('keys');
  ctx = canvas.getContext('2d');
  key_positions = [[100, 100], [180, 100], [260, 100], [340, 100], [100, 180], [180, 180], [260, 180], [340, 220], [100, 260], [180, 260], [260, 260], [100, 340], [180, 340], [260, 340], [340, 380], [140, 420], [260, 420]];
  key_sizes = [[35], [35], [35], [35], [35], [35], [35], [35, 75], [35], [35], [35], [35], [35], [35], [35, 75], [75, 35], [35]];
  _results = [];
  for (i = _i = 0, _len = key_positions.length; _i < _len; i = ++_i) {
    key = key_positions[i];
    _ref = key_sizes[i], size_x = _ref[0], size_y = _ref[1];
    _results.push(draw_curved_rect(ctx, key, size_x, size_y));
  }
  return _results;
};

draw_curved_rect = function(ctx, _arg, size_x, size_y) {
  var bottom, bottom_left, bottom_right, curve_to, left, move_to, point_x, point_x_l, point_x_r, point_y, point_y_b, point_y_t, right, top, top_left, top_right;
  point_x = _arg[0], point_y = _arg[1];
  if (size_y == null) {
    size_y = size_x;
  }
  curve_to = function(_arg1, _arg2) {
    var cp_x, cp_y, x, y;
    cp_x = _arg1[0], cp_y = _arg1[1];
    x = _arg2[0], y = _arg2[1];
    return ctx.quadraticCurveTo(cp_x, cp_y, x, y);
  };
  move_to = function(_arg1) {
    var x, y;
    x = _arg1[0], y = _arg1[1];
    return ctx.moveTo(x, y);
  };
  point_x_l = point_x - size_x;
  point_x_r = point_x + size_x;
  point_y_t = point_y - size_y;
  point_y_b = point_y + size_y;
  top = [point_x, point_y_t];
  left = [point_x_l, point_y];
  right = [point_x_r, point_y];
  bottom = [point_x, point_y_b];
  top_left = [point_x_l, point_y_t];
  top_right = [point_x_r, point_y_t];
  bottom_left = [point_x_l, point_y_b];
  bottom_right = [point_x_r, point_y_b];
  ctx.beginPath();
  move_to(left);
  curve_to(top_left, top);
  curve_to(top_right, right);
  curve_to(bottom_right, bottom);
  curve_to(bottom_left, left);
  return ctx.stroke();
};
