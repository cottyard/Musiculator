...
(function() {
  var Key, key_action_funcs, key_action_pool, key_binds, key_positions, key_sizes, key_texts, keys, music_key_down, music_key_up, note_activators_count, note_key_action, note_offset, play_note, suspend_note, transpose, tune_key_action,
  ...
  keys = function() {
    ...
    keys = (function() {
      ...
      for (i = _i = 0, _len = key_positions.length; _i < _len; i = ++_i) {
        pos = key_positions[i];
        _results.push(new Key(i, ctx, pos, key_sizes[i], key_texts[i], key_binds[i]));
      }
      return _results;
    })();
    _results = [];
  ...
  key_binds = [[], ['0', 'NUM_/'], ['-', 'NUM_*'], ['=', 'NUM_-'], ['7', 'NUM_7'], ['8', 'NUM_8'], ['9', 'NUM_9'], ['S', 'NUM_+'], ['4', 'NUM_4'], ['5', 'NUM_5'], ['6', 'NUM_6'], ['1', 'NUM_1'], ['2', 'NUM_2'], ['3', 'NUM_3'], ['X', 'NUM_return'], ['A', 'NUM_0'], ['Z', 'NUM_.']];

  Key = (function() {
    function Key(id, ctx, position, size, text, binds) {
      var key_bind, _i, _j, _len, _len1, _ref, _ref1;
      this.id = id;
      this.ctx = ctx;
      ...
      for (_i = 0, _len = _ref.length; _i < _len; _i++) {
        key_bind = _ref[_i];
        window.keyboard.add_press_action(key_bind, this.press);
      }
      _ref1 = this.binds;
      for (_j = 0, _len1 = _ref1.length; _j < _len1; _j++) {
        key_bind = _ref1[_j];
        window.keyboard.add_release_action(key_bind, this.release);
      }
  ...
  play_note = function(note) {
    if (note_activators_count[note] != null) {
      note_activators_count[note]++;
    } else {
      note_activators_count[note] = 1;
    }
    if (note_activators_count[note] === 1) {
      return window.piano.press_note(note);
    }
  };

  suspend_note = function(note) {
    note_activators_count[note]--;
    if (note_activators_count[note] === 0) {
      return window.piano.release_note(note);
    }
  };
